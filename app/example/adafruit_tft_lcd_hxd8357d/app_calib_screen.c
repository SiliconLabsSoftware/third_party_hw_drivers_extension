  ...
  uint16_t x_raw, uint16_t y_raw, uint16_t z1_raw, uint16_t z2_raw)
{
  ...
  glib_draw_string(g_context, string_buf, 0, Y_OFFSET + 30);
  snprintf(string_buf, sizeof(string_buf), "X_RAW:  %4d  ", x_raw);
  glib_draw_string(g_context, string_buf, 95, Y_OFFSET + 30);

  snprintf(string_buf, sizeof(string_buf), "Y:  %3ld", y);
  glib_draw_string(g_context, string_buf, 0, Y_OFFSET + 50);
  snprintf(string_buf, sizeof(string_buf), "Y_RAW:  %4d", y_raw);
  glib_draw_string(g_context, string_buf, 95, Y_OFFSET + 50);

  snprintf(string_buf, sizeof(string_buf), "RT: %3ld  ", rtouch);
  glib_draw_string(g_context, string_buf, 0, Y_OFFSET + 70);
  snprintf(string_buf, sizeof(string_buf), "Z1_RAW: %4d  ", z1_raw);
  glib_draw_string(g_context, string_buf, 95, Y_OFFSET + 70);
  snprintf(string_buf, sizeof(string_buf), "Z2_RAW: %4d  ", z2_raw);
  glib_draw_string(g_context, string_buf, 95, Y_OFFSET + 90);
}

void calib_screen_init(glib_context_t *g_context)
{
  glib_fill(g_context, HXD8357D_BLACK);
  glib_draw_string(g_context, "~TOUCH SCREEN CALIB~", 40, Y_OFFSET);
