...
void app_init(void)
{
  ...
  mikroe_eink_display_set_lut(EINK154_LUT_TABLE, 30);

  glib_fill(&g_context, EINK_DISPLAY_SCREEN_COLOR_WHITE);
  glib_draw_xbitmap(&g_context,
                    52, 82, silicon_labs_logo_96x35,
                    96, 35, EINK_DISPLAY_SCREEN_COLOR_BLACK);
  glib_draw_string(&g_context, str1,
                   (MIKROE_EINK_DISPLAY_WIDTH - strlen(str1) * 6 + 1) / 2, 140);
  glib_draw_string(&g_context, str2,
                   (MIKROE_EINK_DISPLAY_WIDTH - strlen(str2) * 6 + 1) / 2, 160);
  glib_update_display();
}
